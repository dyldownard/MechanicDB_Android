...
class GarageRecyclerAdapter(private val onClickListener: OnClickListener) : ListAdapter<Vehicle, GarageRecyclerAdapter.VehicleViewHolder>(VehicleComparator()) {
    ...
    class VehicleViewHolder constructor(
        ...
        val vehicle_make: TextView = itemView.taskNameText
        val vehicle_model: TextView = itemView.taskCompByText
        val vehicle_year: TextView = itemView.taskOdoText
        val vehicle_milage: TextView = itemView.taskDateCompleteText

        fun bind(vehicle: Vehicle) {
            vehicle_make.text = "Make: ".plus(vehicle.make)
            vehicle_model.text = "Model: ".plus(vehicle.model)
            vehicle_year.text = "Year: ".plus(vehicle.year.toString())
            vehicle_milage.text = "Miles: ".plus(vehicle.milage.toString())
            vehicle_image.apply {
